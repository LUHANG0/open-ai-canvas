...
影策（`LUHANG0/open-ai-canvas`）是面向 AI 影视与短剧创作的工作台，当前仍在快速开发。公开接口、数据结构和部署配置可能直接调整；除非任务明确要求，不为旧字段、旧 API 或旧数据增加兼容层。

### 维护基线与分支

- `origin/main` 是唯一持续维护、测试和部署的代码基线；开始新任务前必须确认当前提交来自该分支。
- `ddcat-ai/open-ai-canvas` 仅作为上游来源和版权归属记录。除非用户明确要求并建立独立集成分支，不得自动拉取、合并、覆盖或部署上游代码。
- `feat/*`、`fix/*`、`refactor/*`、`perf/*` 和 `test/*` 是临时工作分支；完整合入 `main` 并完成验证后应删除，不能作为长期部署入口。
- 部署前必须核对后端健康信息中的 commit 与目标 `main` 提交一致；安装脚本、镜像和更新器默认来源必须属于 `LUHANG0/open-ai-canvas`。
