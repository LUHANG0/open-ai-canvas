...
export function ChapterStoryboardGenerationDialog({ open, selectedUnit, effectiveConfig, selectedTextModel, onTextModelChange, onClose, onSubmit, storyboardImpact, availableSkills, skillsLoading, selectedSkillIds, onSkillIdsChange }: ModelDialogProps & { storyboardImpact: { shotCount: number }; availableSkills: Skill[]; skillsLoading: boolean; selectedSkillIds: string[]; onSkillIdsChange: (skillIds: string[]) => void }) {
    return (
        <DialogFrame className="pc-project-dialog" title="生成章节分镜" subtitle="生成成功后会写入分镜制作；已有镜头仅在确认后整体替换。" open={open} frameSize="md" okText={storyboardImpact.shotCount ? "重新生成分镜" : "生成分镜"} cancelText="取消" okButtonProps={{ disabled: !selectedTextModel }} onCancel={onClose} onOk={onSubmit}>
            <div className="grid gap-4">
                <div className="rounded-lg border border-border/70 bg-foreground/[.018] px-3 py-2.5">
                    <div className="text-[var(--fs-tiny)] text-foreground/42">当前章节</div>
                    <div className="mt-1 truncate text-sm font-medium text-foreground/85">{selectedUnit?.title}</div>
                ...
                <label className="block">
                    <span className="mb-1.5 block text-xs font-medium text-foreground/68">文本模型</span>
                    <ModelPicker config={effectiveConfig} capability="text" value={selectedTextModel} onChange={onTextModelChange} fullWidth placeholder="选择用于生成分镜的文本模型" showSelectedPrice={false} onMissingConfig={() => navigateToSettings({ continueCreation: true })} />
                </label>
                <div>
                    <label className="block">
...
export function MoveChapterDialog({ open, position, chapterCount, loading, onPositionChange, onClose, onSubmit }: { open: boolean; position: number | null; chapterCount: number; loading: boolean; onPositionChange: (position: number | null) => void; onClose: () => void; onSubmit: () => void }) {
    return (
        <DialogFrame className="pc-project-dialog" title="移动章节" subtitle="其他章节会按目标位置自动顺延。" open={open} frameSize="sm" okText="移动" cancelText="取消" okButtonProps={{ disabled: !position || position < 1 || position > chapterCount, loading }} onCancel={onClose} onOk={onSubmit}>
            <div className="text-xs leading-5 text-foreground/50">输入目标章节位置。适合上千章项目的长距离调整，移动后其他章节会自动顺延。</div>
            <label className="mt-3 flex items-center gap-2 text-sm"><span className="shrink-0">移动到第</span><InputNumber min={1} max={chapterCount} precision={0} value={position} onChange={onPositionChange} className="min-w-0 flex-1" /><span className="shrink-0">章</span></label>
        </DialogFrame>
    );
}
