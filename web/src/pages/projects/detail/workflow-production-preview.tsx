...
export function WorkflowArtifactPreviewPanel({ activeStage, projectId, unitId, selectedShot, shotTask, artifacts, newestArtifact, previewArtifact, previewTab, resolution, imageQuality, generating, onPreviewTabChange, onSelectArtifact, onGenerate }: Props) {
        ...
        <aside className="workflow-preview-panel">
            <header className="workflow-preview-header">
                <div className="workflow-preview-header-row">
                    ...
                    <Segmented size="small" value={previewTab} onChange={(value) => onPreviewTabChange(value as typeof previewTab)} options={[{ value: "latest", label: "最新" }, { value: "history", label: `历史 ${artifacts.length}` }]} />
                </div>
                <Segmented
                    block
                    size="small"
            ...
            <div className="workflow-preview-scroll thin-scrollbar">
                {shotTask?.status === "failed" || shotTask?.status === "cancelled" ? <Alert className="mb-3" type={shotTask.status === "failed" ? "error" : "warning"} showIcon message={shotTask.status === "failed" ? "上次生成失败" : "上次生成已取消"} description={shotTask.error ? generationErrorMessage(shotTask.error) : "可以检查模型与参考资产后重新提交。"} action={<Button size="small" onClick={onGenerate}>重试</Button>} /> : null}
                {previewTab === "latest" ? <LatestPreview artifact={previewArtifact} emptyText={stageCopy.empty} /> : <ArtifactHistory artifacts={artifacts} activeId={previewArtifact?.id} onSelect={(artifact) => { onSelectArtifact(artifact); onPreviewTabChange("latest"); }} />}
                ...
                <div className="workflow-preview-actions"><Button icon={<RefreshCcw className="size-3.5" />} loading={generating} onClick={onGenerate}>重新生成</Button><Button icon={<Download className="size-3.5" />} disabled={!previewArtifact?.resourceId} onClick={() => previewArtifact?.resourceId && void downloadArtifact(previewArtifact, selectedShot.title, message.error)}>下载{activeStage === "video" ? "视频" : "图片"}</Button></div>
                <ArtifactHistory artifacts={artifacts.slice(0, 4)} activeId={previewArtifact?.id} onSelect={onSelectArtifact} compact />
            </div>
        </aside>
    );
}
