...
import { DialogFrame, FormSection, SectionHeader, StatusBadge } from "@/components/ui/pc";
...
export default function ProjectSettingsView({ detail, refreshProject }: ProjectDetailViewProps) {
    ...
    return (
        <div className="pc-project-settings">
            <SectionHeader
                className="pc-project-settings-header"
                title="项目设置"
                description="维护基础信息、默认生成模型、项目主图、画风快照与归档状态。"
                meta={<StatusBadge tone={dirty ? "warning" : "success"} dot>{dirty ? "有未保存修改" : "设置已同步"}</StatusBadge>}
                actions={<Button type={dirty ? "primary" : "default"} icon={dirty ? <Save className="size-3.5" /> : <Check className="size-3.5" />} disabled={!dirty || !name.trim()} loading={saveMutation.isPending} onClick={() => saveMutation.mutate()}>{dirty ? "保存设置" : "已保存"}</Button>}
            />

            <FormSection className="pc-project-settings-section" title="基础设置" description="项目名称、画幅和内容来源会作为后续制作的默认上下文。">
            ...
            <FormSection className="pc-project-settings-section" title="默认生成模型" description="分镜图、动作预演与镜头视频优先使用项目默认；未设置或模型不可用时跟随工作台全局默认。">
            ...
            <FormSection className="pc-project-settings-section" title="项目主图" description="项目列表优先展示这张图片；未设置时使用项目画风示意图。" requiredHint="建议与项目画幅一致">
            ...
            <FormSection className="pc-project-settings-section" title="项目画风" description="项目保存当前版本快照；修改“我的风格”不会自动改写历史项目。" requiredHint={styleProfile ? styleProfile.source === "user" ? "来自我的风格" : styleProfile.source === "external" ? "外部导入" : "系统预设" : "未设置"}>
            ...
            <FormSection className="pc-project-settings-section pc-project-settings-danger" title="项目状态" description="归档会停止项目内新建与生成，但不会删除已有内容。">
