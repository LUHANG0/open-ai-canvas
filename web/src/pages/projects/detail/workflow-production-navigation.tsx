...
export function ShotTimeline({
    ...
    const readyCount = shots.filter((shot) => currentArtifact(detail, shot.id, artifactType)?.status === "ready").length;
    return (
        <section className="workflow-shot-timeline">
            <header>
                <div className="workflow-shot-timeline-title">
                ...
                <div className="workflow-shot-timeline-summary">
                    <span>
                        {stageLabel} · {readyCount}/{shots.length} 就绪
                    </span>
                    <span>
