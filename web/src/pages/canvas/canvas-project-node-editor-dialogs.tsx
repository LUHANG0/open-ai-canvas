...
export function CanvasProjectNodeEditorDialogs({
    ...
    return (
        <>
            {characterReferenceNode ? (
                <Suspense fallback={<CanvasWorkspaceLoadingOverlay theme={theme} title="正在加载角色参考" description="正在准备角色参考编辑器。" />}>
                    <CanvasCharacterReferenceModal node={characterReferenceNode} open onClose={onCloseCharacterReference} />
                </Suspense>
            ) : null}
            {textEditorNode ? (
                <Suspense fallback={<CanvasWorkspaceLoadingOverlay theme={theme} title="正在加载文本编辑器" description="正在准备文本内容。" />}>
            ...
            {drawingNode ? (
                <Suspense fallback={<CanvasWorkspaceLoadingOverlay theme={theme} title="正在加载绘图编辑器" description="正在准备绘图画布。" />}>
            ...
            {portraitClearanceNode ? (
                <Suspense fallback={<CanvasWorkspaceLoadingOverlay theme={theme} title="正在加载肖像排查" description="正在准备人物肖像分析。" />}>
