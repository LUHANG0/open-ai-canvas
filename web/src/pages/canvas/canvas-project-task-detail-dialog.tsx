...
export type CanvasStatusDialogTheme = { node: { stroke: string; panel: string; muted: string; fill: string } };

type CanvasProjectTaskDetailDialogProps = {
...
export function CanvasProjectTaskDetailDialog({ theme, task, taskLogs, taskLoading, onClose, onCancel }: CanvasProjectTaskDetailDialogProps) {
    const config = useEffectiveConfig();
    return (
        <Modal rootClassName="pc-canvas-overlay pc-canvas-modal pc-canvas-task-modal" title="任务详情" open footer={null} width="min(920px, calc(100vw - 32px))" onCancel={onClose}>
            <div className="pc-canvas-task-detail space-y-4 text-sm" aria-busy={taskLoading}>
                ...
                <div>
                    ...
                    <div className="pc-canvas-task-detail__prompt h-32 overflow-y-auto whitespace-pre-wrap rounded-lg p-3 text-xs leading-5" style={{ background: theme.node.fill }}>{task.prompt || "未记录"}</div>
                </div>
                <TaskGenerationParameters inputJson={task.inputJson} theme={theme} />
                {onCancel && (task.status === "queued" || task.status === "running") ? (
                    <div className="flex justify-end"><Button danger icon={<XCircle className="size-4" />} onClick={() => onCancel(task)}>取消任务</Button></div>
                ) : null}
                <div>
...
function TaskGenerationParameters({ inputJson, theme }: { inputJson?: string; theme: CanvasStatusDialogTheme }) {
    ...
    return (
        <div>
            ...
            ) : <div className="pc-canvas-task-detail__parameters-empty rounded-lg p-3 text-xs" style={{ background: theme.node.fill, color: theme.node.muted }}>暂无参数记录</div>}
        </div>
    );
}
