    ...
    return (
        <main className={`brand-loading-lab${still ? " no-motion" : ""}`}>
            ...
            <section className="brand-loading-lab-hero" aria-label="品牌开场预览">
                ...
                <Button className="brand-loading-lab-fullscreen" onClick={() => setFullscreen(true)}>
                    全屏预览
                </Button>
            </section>
            <div className="brand-loading-lab-grid">
                <section>
                    <h2>页面切换</h2>
                    <div className="brand-loading-lab-route">
                        <Suspense key={`${cycle}-${delay}`} fallback={<WorkspaceRouteLoader />}>
                            <Resolved />
                        </Suspense>
                    </div>
                    <div className="brand-loading-lab-actions">
                        <Button
                            onClick={() => {
                                setDelay(1600);
                                setCycle(cycle + 1);
                ...
                <section>
                    <h2>局部与按钮</h2>
                    <div className="brand-loading-lab-inline">
                        <Spin size="small" />
                        <Spin />
                        <Spin size="large" />
                        <BrandLoadingIndicator size="md" />
                    </div>
                    <div className="brand-loading-lab-actions">
                        <Button type="primary" loading>
                            正在保存
                        </Button>
                        <Button loading>正在读取</Button>
                    </div>
                </section>
            </div>
            <section className="brand-loading-lab-data">
                <WorkspaceLoadingState label="正在整理素材" detail="保留内容骨架，加载完成后直接呈现" rows={3} />
            </section>
            {fullscreen ? (
                <>
                    <FullScreenLoader />
                    <Button className="brand-loading-lab-close" onClick={() => setFullscreen(false)}>
                        退出全屏预览
                    </Button>
                </>
            ) : null}
