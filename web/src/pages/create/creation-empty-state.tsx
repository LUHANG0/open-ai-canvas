        ...
        <header className="creation-empty-intro">
            ...
            <h1>
                ...
                <span className="creation-empty-title-pc">
                    让灵感成为<strong>{modeHeadline[mode]}</strong>
                </span>
            </h1>
            <p className="creation-empty-description-mobile">
                从一个想法开始，和{branding.config.identity.shortName}一起完成你的{modeHeadline[mode]}。
            </p>
            <p className="creation-empty-description-pc">写下想法，选择模型。从第一版开始，逐步打磨你的{modeHeadline[mode]}。</p>
        </header>
    );
}
...
export function CreationEmptySuggest({ onStartPrompt }: { onStartPrompt: (mode: CreationMode, prompt: string) => void }) {
    return (
        <section className="creation-empty-starters" aria-label="创作灵感推荐">
