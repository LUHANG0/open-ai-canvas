...
export type CanvasAssetTrayContentProps = {
    open: boolean;
    initialTab: TrayTab;
...
export function CanvasAssetTrayContent({ open, initialTab, assetImages, canvasImages, showLibrary, activeNodeId, dndType, theme, onClose, onInsertAssetImage, onFocusCanvasImage }: CanvasAssetTrayContentProps) {
    const reducedMotion = useReducedMotion();
    const [tab, setTab] = useState<TrayTab>(initialTab);
    const [keyword, setKeyword] = useState("");
    const [trayHeight, setTrayHeight] = useState(() => clampTrayHeight(520));
    ...
    const startAssetDrag = (event: DragEvent<HTMLElement>, asset: ImageAsset) => {
        ...
        event.dataTransfer.setData(dndType, asset.id);
