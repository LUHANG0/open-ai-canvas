...
import { getGenerationResourceNodes, getContextResourceNodes, getMentionResourceNodes, type CanvasResourceGraphIndex } from "@/lib/canvas/canvas-resource-references";
...
function buildComposerGenerationContext(inputs: NodeGenerationInput[], prompt: string, videoFrameNodeIds: string[] = [], autoIncludeWorkflowMedia = false, workflowMediaInputs: NodeGenerationInput[] = []): NodeGenerationContext {
...
export function buildNodeGenerationInputs(nodeId: string, nodes: CanvasNodeData[], connections: CanvasConnection[], graphIndex?: CanvasResourceGraphIndex): NodeGenerationInput[] {
    return buildGenerationInputs(getGenerationResourceNodes(nodeId, nodes, connections, graphIndex), nodes, connections);
}
