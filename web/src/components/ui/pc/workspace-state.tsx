...
export function WorkspaceState({ icon = "empty", title, description, action, compact = false, className, role }: WorkspaceStateProps) {
    ...
    return (
        ...
        >
            ...
            {description ? (
                <p id={descriptionId} className="workspace-state-description mt-1.5 max-w-[42ch] text-xs leading-5 text-foreground/58">
                    {description}
                </p>
            ) : null}
...
export function WorkspaceErrorState({ title = "暂时无法加载", description, actionLabel = "重新加载", onRetry, compact = false }: WorkspaceErrorStateProps) {
...
export function WorkspaceLoadingState({ label = "正在加载内容", detail, rows = 3, className }: WorkspaceLoadingStateProps) {
    const labelId = useId();
        ...
        <section className={cn("workspace-loading-state py-8", className)} role="status" aria-busy="true" aria-live="polite" aria-labelledby={labelId} aria-describedby={detail ? detailId : undefined}>
            <div className="mb-5 flex items-center gap-3">
                ...
                <div>
                    ...
                    {detail ? (
                        <div id={detailId} className="workspace-loading-state-description mt-0.5 text-xs text-foreground/50">
                            {detail}
                        </div>
                    ) : null}
                </div>
            ...
            <div className="workspace-loading-state-grid grid gap-3 sm:grid-cols-2 xl:grid-cols-3" aria-hidden="true">
                {Array.from({ length: rows }, (_, index) => (
                    <div key={index} className="workspace-loading-state-card rounded-md bg-surface-active p-4">
