...
test("canvas node search loads only after the search entry opens", async () => {
    ...
    expect(source).toContain('<Suspense fallback={<CanvasDialogLoadingOverlay label="正在加载节点搜索…" onClose={onClose} />}>');
...
test("canvas script editor loads only while a script node is selected", async () => {
    ...
    expect(source).toContain('<Suspense fallback={<CanvasDialogLoadingOverlay label="正在加载分镜脚本编辑器…" onClose={onClose} />}>');
...
test("secondary editor loading surfaces remain dismissible", async () => {
    const [overlaySource, ...sources] = await Promise.all([
        Bun.file(new URL("../src/pages/canvas/canvas-dialog-loading-overlay.tsx", import.meta.url)).text(),
        Bun.file(new URL("../src/pages/canvas/canvas-project-node-search.tsx", import.meta.url)).text(),
        Bun.file(new URL("../src/pages/canvas/canvas-project-script-editor.tsx", import.meta.url)).text(),
    ]);

    for (const source of sources) {
        expect(source).toContain("onClose={onClose}");
    }
    expect(overlaySource).toContain("onClick={onClose}");
    expect(overlaySource).toContain("关闭");
    expect(overlaySource).toContain('role="status" aria-live="polite"');
});
