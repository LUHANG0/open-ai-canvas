...
describe("canvas skill mentions", () => {
    ...
    it("keeps the canvas compatibility layer free of independent prompt expansion", async () => {
        const source = await Bun.file(new URL("../src/lib/canvas/canvas-skill-mentions.ts", import.meta.url)).text();
        expect(source).toContain('from "@/services/skill-runtime"');
        expect(source).not.toContain("getSkillFile");
        expect(source).not.toContain("renderSkillPrompt");
    });
});
