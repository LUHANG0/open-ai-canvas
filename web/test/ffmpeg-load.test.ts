...
function silentWorker() {
    let terminated = 0;
    return {
        load: (_config: unknown, options?: { signal?: AbortSignal }) => new Promise<boolean>((_resolve, reject) => {
...
test("已取消时不启动 Worker", async () => {
    let loads = 0;
    const worker = { load: async () => { loads += 1; return true; }, terminate: () => {} };
...
test("核心加载错误释放失败实例，下一实例可正常加载且清除计时器", async () => {
    const bad = { load: async () => { throw new Error("wasm 404"); }, terminate: () => {} };
    await expect(loadFFmpegInstance(bad, {})).rejects.toThrow("视频合并工具加载失败，请重试");
    let terminated = 0;
    const controller = new AbortController();
    await loadFFmpegInstance({ load: async () => true, terminate: () => { terminated += 1; } }, {}, controller.signal, 5);
    controller.abort();
