...
实现与证据已快进合入本地 main，3012 长期预览同步源码，原 PID 89878 保持运行。预览在拦截 `/api/` 为隔离配置后实测 HTTP 200、Worker/core/WASM 200、加载成功、零页面异常；没有请求真实后端配置或业务数据。初次临时探针的 API 匹配过宽，误拦截源码目录导致动态导入失败；限定为站点根 `/api/` 后通过，未因此改动产品。

本批 3014/3015 按 PID 与工作目录核对后已停止。完整证据归档到主仓库 `.local/cache/project-delivery-loading-20260905`，逐个核验 1211 个文件的 SHA-256 一致，清单为该目录 `archive-checks.json`。主仓库不含运行数据或媒体提交，既有长期预览的依赖链接保留；未推送或部署。
