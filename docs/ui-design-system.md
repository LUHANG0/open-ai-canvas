...
> 状态：以官网品牌页为主的全站基础规范。用户端、管理后台与移动端共享主题；专用工作台按批次迁移。

## 1. 适用范围

- 官网、认证、用户端和管理后台共享字体、表面、强调色、控件状态与焦点语义。
- `1024px` 是桌面布局断点，不再用于切换品牌主题；移动端保留现有信息架构和交互。
- `/canvas/:id`、公开分享、登录注册可使用独立页面结构，但仍消费统一字体、颜色、焦点、表单和状态 Token。
- `/admin/**` 保留管理表格的密度与专用业务组件；数据库、API 合同、权限和业务规则不因视觉统一而改变。
- 官网四页与登录注册固定深色；工作区和后台尊重用户明暗偏好，路由切换不覆盖偏好。
...
| 角色 | 亮色 | 暗色 |
| --- | --- | --- |
| Canvas | `#f3f4f6` | `#101115` |
| Surface 1 | `#ffffff` | `#191b22` |
| Surface 2 | `#f7f8fa` | `#22252e` |
| Surface 3 | `#eceff3` | `#2b2e38` |

具体颜色必须通过 Semantic Token 消费，页面中不得直接复制表格值。

主操作采用黑白配对；品牌色用于链接、焦点、导航和选择状态。默认深色强调色为 `#b1a4ff`，后台仍保存基础品牌色 `#8B7CF6`。自定义强调色按明暗表面派生并保证至少 4.5:1 的文字对比度，成功、警告、错误和运行状态保留独立语义。Logo 以完整图片呈现，不叠加装饰底色或状态圆点。
...
共用：中性表面、三层文字、细边框、低阴影、224/64 侧栏、壳层留白、焦点和品牌强调色。后台私有 `--admin-*` 变量引用共享语义变量。

保留：后台 34px 菜单密度、管理表单和表格结构、权限与危险操作确认。用户端不依赖 Admin 私有类名；后台覆盖放在 `admin-ui.css`，不在全局样式重新复制。
