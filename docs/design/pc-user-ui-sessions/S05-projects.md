...
3. 项目卡保留原封面、阶段、删除、统计和跳转逻辑；PC 使用语义化 `progress`，1024px 以下保留原进度条 DOM 与视觉。
4. 保留每页 50 条无限加载，以及只对当前已加载项目执行搜索、状态筛选和排序的既有语义。
5. 补齐“完全没有项目”的空状态；此前空状态仅在存在搜索或状态筛选时显示。

### 项目详情

1. 保留 `WorkspacePage` 原有滚动属性与 `data-scroll-owner`，由详情标准页或章节/工作流内部容器分别拥有滚动，避免双滚动。
...
4. PC 使用公共 `StatusBadge` 统一草稿、保存中、待生成、运行、成功、失败和过期状态；1024px 以下通过 Workflow 私有规则恢复旧 Tag 的尺寸、色调和静态外观。
5. 保留任务提交、任务状态轮询、报价、模型兼容、技能运行、素材引用、脚本版本、产物历史、下载与删除确认逻辑。

### PC-only 边界加固

1. 新增 Token 重映射、六阶段导航和 PC 表面规则全部收口到 `@media (min-width: 1024px)`。
2. 项目库在小屏保留原 AntD 搜索框与原进度条，PC 才显示 `SearchField` 和语义化 `progress`。
3. 恢复详情顶栏在 768–1023px 的原宽度、文字、焦点、导航和按钮合同；PC 样式仍由 Projects 专属 CSS 覆盖。
4. 为 `SectionHeader`、`Surface`、`StatTile`、`FormSection`、`StatusBadge` 和 `DialogFrame` 增加 Projects/Workflow 作用域内的 1024px 以下等价保护。
5. 恢复 ModelPicker 原焦点环与绑定素材 40px Ant Image 约束，不改工作流交互。
...
- `1e4af34` · `fix(pc-ui): projects - preserve mobile contracts`
...
- Project Assets、Settings 的 PC 结构已重排；1024px 以下已加入旧版等价样式与公共组件保护，仍需在 R7 做真实浏览器视觉回归。
...
- `git diff --check`：四个代码提交前均通过。
- PC-only 保护检查：`projects.css` 新视觉默认位于 `min-width: 1024px`；Workflow 根 Token、阶段导航、Surface 和状态规则已分离为 PC 规则与 `max-width: 1023px` 旧版保护。
- `1e4af34` 提交前 `git diff --check` 通过；变更仅位于 `web/src/pages/projects/**`。
...
1. `git revert 1e4af34`：仅回滚 PC-only/小屏旧版保护加固。
2. `git revert 1a6ae34`：回滚六阶段导航、公共状态与 Workflow 视觉。
3. `git revert baf9eec`：回滚项目详情壳层和各详情视图。
4. `git revert ebb6f47`：回滚项目库、创建弹窗和页面私有样式文件。
5. 本记录可随所属文档提交单独 `git revert <S05-文档-SHA>`。
6. 不使用 `git reset --hard` 或强制推送。
