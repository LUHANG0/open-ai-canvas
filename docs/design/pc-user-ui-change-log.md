...
| S13  | R7 集中测试与修复                       | `refactor/pc-user-ui-20260901`       | `217a022`  | `2fae2e3`、`e5c80bc`、`4705169`            | 官方测试/类型/构建/Director E2E 通过；修复 Token 计费展示、提示布局和素材批量文案契约         | 直接提交  | 按 `4705169` → `e5c80bc` → `2fae2e3` 逆序 `git revert` |
| S14  | R8 主分支落地与本地部署验证             | `main`                               | `f41be733` | `c8751e3` + 部署记录提交                   | `--ff-only` 合并；仅替换 8888 前端，8080 后端 PID 不变；10 个 PC 主路由部署后冒烟通过         | 快进合并  | 参考部署记录与 R0–R8 标签                              |
...
- 官方 1116 条测试、类型检查、生产构建和 Director Chrome E2E 58/58 全部通过。
- 真实浏览器覆盖 768/1024/1440/1920px、亮暗主题、用户主路由、Admin 只读概览、公开分享错误页和 404。
- 相对 R0，backend、Admin、services、stores 和路由合同零差异；最终验证限制与历史债务记录于集中测试报告。

## R8 本地部署结论

- `main` 已从 R0 基线通过 `--ff-only` 快进到 `c8751e3`，两个工作树均干净，未推送远端。
- 原目录生产构建通过；只替换 `yingce-web` 的 8888 前端服务，8080 后端仍为原 PID `21178`。
- HTTP 健康检查、创作页关键入口和 10 个 PC 主路由浏览器冒烟均通过。
- 具体进程、端口、验证结果和恢复方式见 `pc-user-ui-deployment-record.md`。
