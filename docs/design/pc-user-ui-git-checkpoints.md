...
| R7     | 集中测试与修复完成                | `pc-user-ui-r7-20260901`         | 已建立于 `4705169`；官方测试、类型、构建、Director E2E 与浏览器回归完成               |
| R8     | 最终交付候选                      | `pc-user-ui-r8-20260901`         | 本轮交付文档提交后建立；包含测试报告、已知问题、台账和回滚记录                        |
| D1     | 主分支落地与本地部署验证          | `pc-user-ui-deployed-20260901`   | 建立于部署记录提交；`main` 快进到 R8，8888 原位替换，8080 后端保持不变                |
...
- D1 只增加部署记录，不改变 R8 业务代码。
- 部署服务与验证明细见 `pc-user-ui-deployment-record.md`。
- `pc-user-ui-r8-20260901` 用于定位交付代码，`pc-user-ui-deployed-20260901` 用于定位本次实际落地记录。
